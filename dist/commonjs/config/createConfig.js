...
var createConfig = function createConfig(userConfig) {
  ...
  */
  ...
  if (!process.browser && typeof window === 'undefined') {
      ...
      if (typeof serverLocalePath === 'string') {
        ...
        combinedConfig.backend = {
          addPath: function addPath(locale, namespace) {
            return serverLocalePath(locale, namespace, 'server', true);
          },
          loadPath: function loadPath(locale, namespace) {
            return serverLocalePath(locale, namespace, 'server', false);
          }
        };
      } //
    ...
    //
    var clientLocalePath = typeof localePath === 'string' && localePath.match(/^\.?\/public\//) ? localePath.replace(/^\.?\/public/, '') : localePath; //
    ...
    if (!hasCustomBackend) {
      if (typeof clientLocalePath === 'string') {
        ...
        combinedConfig.backend = {
          addPath: function addPath(locale, namespace) {
            return clientLocalePath(locale, namespace, 'client', true);
          },
          loadPath: function loadPath(locale, namespace) {
            return clientLocalePath(locale, namespace, 'client', false);
          }
        };
      }
    }
