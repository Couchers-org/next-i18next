  ...
  if (!process.browser && typeof window === 'undefined') {
    ...
    if (!hasCustomBackend) {
      ...
      if (typeof serverLocalePath === 'string') {
        ...
        combinedConfig.backend = {
          addPath: (locale, namespace) => serverLocalePath(locale, namespace, 'server', true),
          loadPath: (locale, namespace) => serverLocalePath(locale, namespace, 'server', false)
        };
      } //
      ...
      if (!combinedConfig.ns && typeof lng !== 'undefined') {
        if (typeof serverLocalePath === 'function') throw new Error('Must provide all namespaces in ns option if using a function as localePath');
    ...
    //
    const clientLocalePath = typeof localePath === 'string' && localePath.match(/^\.?\/public\//) ? localePath.replace(/^\.?\/public/, '') : localePath; //
    ...
    if (!hasCustomBackend) {
      if (typeof clientLocalePath === 'string') {
        ...
        combinedConfig.backend = {
          addPath: (locale, namespace) => clientLocalePath(locale, namespace, 'client', true),
          loadPath: (locale, namespace) => clientLocalePath(locale, namespace, 'client', false)
        };
      }
    }
